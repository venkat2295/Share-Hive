...
export const getUserPosts = async (token, searchKeyword = "", page = 1, limit = 10) => {
  try {
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    const { data, headers } = await axios.get(
      `/api/posts/user?searchKeyword=${searchKeyword}&page=${page}&limit=${limit}`,
      config
    );
    return { data, headers };
  } catch (error) {
...
export const getUserPost = async ({ slug, token }) => {
  try {
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    const { data } = await axios.get(`/api/posts/user/${slug}`, config);
    return data;
  } catch (error) {
    if (error.response && error.response.data.message)
      throw new Error(error.response.data.message);
...
export const updateUserPost = async ({ updatedData, slug, token }) => {
  try {
    ...
    const { data } = await axios.put(`/api/posts/user/${slug}`, updatedData, config);
    return data;
  } catch (error) {
    if (error.response && error.response.data.message)
      throw new Error(error.response.data.message);
